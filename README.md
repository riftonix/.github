# RiftOniX 

Tools and shared automation for DevOps, SRE, and platform engineers. RiftOniX
projects focus on reproducible CI, cloud-native infrastructure, and Kubernetes
delivery.

We build small, composable tools that make engineering workflows easier to
repeat across repositories and environments.

## Shared Automation Home

This `.github` repository is the shared automation home for the Riftonix
organization. GitHub gives it organization-level discovery and default
behavior, so reusable workflows, composite actions, Renovate presets, and
migration documentation have one canonical location.
